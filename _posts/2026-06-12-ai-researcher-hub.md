...
<section>
    <h3 style="text-align: center;">&sect;3 For an agent, the full record is nearly free</h3>

    <p>What is changing the situation is the AI agent. Far from perfectly, but increasingly in practice, agents can now execute parts of the research process automatically &mdash; literature surveys, implementing and running experiments, analysis, writing. And here a simple fact matters: <strong>an agent's activity happens entirely on a computer</strong>. Every tool call, every file edit, every command run is already a machine event, so the agent's execution history can be captured as a log essentially as a side effect &mdash; nobody has to do the work of recording. The total record that never paid off for humans comes almost for free with an agent.</p>
