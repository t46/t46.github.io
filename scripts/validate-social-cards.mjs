...
const [pagesDirectory = "_site/blogs", siteRoot = "_site", siteOrigin = "https://t46.github.io"] = process.argv.slice(2);
...
for (const page of pages) {
  ...
  for (const key of ["og:url", "og:image", "twitter:image"]) {
    if (metadata[key] && !metadata[key].startsWith("https://")) {
      failures.push(`${page}: ${key} must use an absolute HTTPS URL`);
    }
  }

  const image = metadata["og:image"];
  if (image?.startsWith(`${siteOrigin}/`)) {
    const imagePath = path.join(siteRoot, decodeURIComponent(new URL(image).pathname).replace(/^\/+/, ""));
    if (!fs.existsSync(imagePath)) {
      failures.push(`${page}: social image does not exist at ${imagePath}`);
    } else {
      const size = imageDimensions(imagePath);
      if (!size || size.width < 600 || size.height < 315) {
        failures.push(`${page}: social image must be a PNG or JPEG of at least 600×315 pixels`);
      }
    }
  }
}
...
console.log(`Validated social previews for ${pages.length} blog posts.`);
